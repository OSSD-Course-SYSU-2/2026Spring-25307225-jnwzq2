...
一个基于 HarmonyOS 开发的五子棋游戏，支持跨设备协同、无缝流转体验和人机对战功能。

## 项目简介

本项目是一个完整的五子棋游戏应用，集成了 HarmonyOS 的分布式能力，实现了"一次开发多端部署"和"自由流转"两大核心特性。玩家可以在手机、平板、PC 等多种设备上无缝切换游戏，享受一致的游戏体验。同时支持人机对战模式，提供三档难度的AI对手。
...
- **战术组合**：合理搭配技能扭转战局
- **AI技能使用**：人机对战中AI会主动使用技能

### 🤖 AI系统

#### AI特性
- **智能落子**：基于位置评估算法，考虑进攻和防守价值
- **技能决策**：根据局势和能量主动使用技能
- **思考模拟**：模拟思考时间，提升游戏体验
...
| 难度 | 落子策略 | 技能使用 | 适合人群 |
|------|----------|----------|----------|
| 🌱 简单 | 随机选择前5候选位置 | 10%概率尝试 | 新手玩家 |
| ⚡ 中等 | 随机选择前3候选位置 | 30%概率尝试 | 普通玩家 |
| 🔥 困难 | 选择最佳位置 | 50%概率尝试 | 高手挑战 |

#### AI算法
- **位置评估**：综合考虑进攻价值、防守价值、中心优势
- **棋型识别**：识别成五、活四、冲四、活三、眠三等棋型
- **技能策略**：
  - 画地为牢：封锁对手关键位置
  - 飞沙走石：移除对手重要棋子
...
│   └── pages/
│       ├── Index.ets              # 主页面（对战模式选择）
│       ├── LocalBattlePage.ets    # 本地对战页面
│       ├── AIBattlePage.ets       # 人机对战页面
...
#### 技术改进
- 📁 新增 LocalBattlePage.ets 和 AIBattlePage.ets
- 🔧 优化路由参数传递机制
- 🎨 AI思考状态UI提示
- 🚀 改进游戏状态管理
...
```typescript
// 自定义位置评估函数
evaluatePositionCustom(row: number, col: number): number {
  let score = 0;
  // 添加自定义评估逻辑
  return score;
}
...
// 调整AI难度参数
const aiConfig = {
  easy: { thinkTime: 300, randomFactor: 5 },
  medium: { thinkTime: 500, randomFactor: 3 },
  hard: { thinkTime: 800, randomFactor: 1 }
};
```
